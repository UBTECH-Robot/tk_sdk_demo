...
# 速度模式安全参数
VELOCITY_MODE_DURATION = 1.0  # 速度模式持续时间（秒）- 防止电机无限运转
...
class HeadMotorController(Node):
    ...
    def set_zero(self):
        ...
        joint_ids = ", ".join(str(motor_id) for motor_id in HEAD_MOTOR_IDS)
        ...
        self.get_logger().warn("✓ 请等待驱动器完成标零并手动重启 proc_manager 服务或直接断电重启机器人(如果是仅启动body_control服务的话则直接重启body_control服务)")
