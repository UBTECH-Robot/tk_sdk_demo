...
使用方法(要先确保机器人是固定在移位机上，本体服务是启动着的，但是不能启动运控服务，也就是手动关掉 proc_manager 服务后再执行 ros2 launch body_control body.launch.py)：
    执行位置控制模式示例（默认）：
        ros2 run sdk_demo leg_motor_control pos
...
class LegMotorController(Node):
    ...
    def __init__(self):
        ...
        功能：
            1. 调用父类构造函数，创建名为'leg_motor_controller'的节点
            2. 创建三个发布者，分别发送三种不同控制模式的命令
            3. 记录当前电机位置（用于位置模式的相对运动）
        """
        # 初始化节点，节点名称为'leg_motor_controller'
        super().__init__('leg_motor_controller')
        
        # 创建位置模式命令发布者
        # 发布到 /leg/cmd_pos 话题，消息类型为 CmdSetMotorPosition
        self.pos_cmd_publisher = self.create_publisher(
            CmdSetMotorPosition,
            '/leg/cmd_pos',
            10
        )
        self.get_logger().info("✓ 位置模式发布者已创建（话题：/leg/cmd_pos）")
        
        # 创建力位混合模式命令发布者
        # 发布到 /leg/cmd_ctrl 话题，消息类型为 CmdMotorCtrl
        self.ctrl_cmd_publisher = self.create_publisher(
            CmdMotorCtrl,
            '/leg/cmd_ctrl',
            10
        )
        self.get_logger().info("✓ 力位混合模式发布者已创建（话题：/leg/cmd_ctrl）")
        
        # 创建速度模式命令发布者
        # 发布到 /leg/cmd_vel 话题，消息类型为 CmdSetMotorSpeed
        self.vel_cmd_publisher = self.create_publisher(
            CmdSetMotorSpeed,
            '/leg/cmd_vel',
            10
        )
        self.get_logger().info("✓ 速度模式发布者已创建（话题：/leg/cmd_vel）")
        
        # 创建标零命令发布者（需要配合标零工具使用）
        # 发布到 /leg/cmd_set_zero 话题，用于将指定关节的当前位置标记为零位
        self.zero_cmd_publisher = self.create_publisher(
            String,
            '/leg/cmd_set_zero',
            10
        )
        self.get_logger().info("✓ 标零发布者已创建（话题：/leg/cmd_set_zero）")
                
        self.get_logger().info("=" * 50)
        self.get_logger().info("双腿电机控制节点已初始化完成")
        self.get_logger().info("=" * 50)
    ...
    def velocity_control_mode(self):
        ...
        while True:
            ...
            # 为每个电机创建控制命令
            self.get_logger().info("\n发送运动命令...")
            for i, motor_id in enumerate(leg_MOTOR_IDS):
                # 创建单个电机的速度命令
                cmd = SetMotorSpeed()
                cmd.name = motor_id  # 电机ID
    ...
    def set_zero(self):
        ...
        self.get_logger().warn("⚠️  1. 所有双腿关节已使用标零工具物理固定在设计的零位")
        self.get_logger().warn("⚠️  2. 机器人已安全固定在移位机上")
        self.get_logger().warn("⚠️  3. 周围没有人员")
        self.get_logger().warn("⚠️  4. 所有其他控制程序已停止")
        self.get_logger().warn("")
        
        joint_ids = ", ".join(str(motor_id) for motor_id in leg_MOTOR_IDS)
        ...
        self.get_logger().info("发送标零命令到所有双腿关节...")
